...
public parseOptionData(response :any) {
  ...
  const options = doc.querySelectorAll('option') ?  doc.querySelectorAll('option') : null;

  this.dataDongXe = Array.from(options).map(option => ({
    id: parseInt(option.getAttribute('value')) ? parseInt(option.getAttribute('value')) : null,
